{
	...
	"Create Menu": {
		...
		"body": [
			...
			"",
			"",
			"SELECT MAX(MENUKEY) FROM UMF_MENUDEF;-- L?Y GI? TR? MAX C?A MENUKEY",
			"-- L?u ?: T?n menu kh?ng ???c c? space (k? c? 1 byte l?n 2 bytes)",
			"",
			"",
	...
	"Create Schema": {
		...
		"body": [
			"SET CLIENT_ENCODING TO 'SJIS';",
			"",
			"-- DROP SCHEMA PXC_AB17_001 CASCADE;",
			"CREATE SCHEMA IF NOT EXISTS PXC_AB17_001 AUTHORIZATION ntuser;",
			"COMMENT ON SCHEMA PXC_AB17_001",
			"IS '",
			"\t[サブ機能  ] AB.基本マスタ",
			"\t[機能ＩＤ  ] AB17",
			"\t[機能名称  ] 発注先/支払先マスタ一覧",
			...
			"\t[リビジョン] 1",
			"\t[作 成 日  ] 2022/01/17",
			"\t[作 成 者  ] UNICCS/n1-dung",
			"\t[関数一覧  ] 1.f_GetOtherAuthFlag",
			"\t             2.f_InsertErrorData",
			"\t             3.f_InsertTmpPayPartnerSight",
			"\t             4.p_CsvEntry_Partner_PayPartner",
			"\t             5.p_CsvEntry_Partner",
			"\t             6.p_CsvEntry_PayPartner",
			"\t             7.p_CsvImportCheck_Partner_PayPartner",
			"\t             8.p_CsvImportCheck_Partner",
			"\t             9.p_CsvImportCheck_PayPartner",
			"\t             10.p_GridSearch",
			"\t             11.p_Export_Partner_ProcessFee",
			"\t             12.p_Export_Partner",
			"\t             13.p_Export_PayPartner",
			"\t[更新履歴  ] ",
			"';",
			"",
			"RESET CLIENT_ENCODING;"
	...
	"Get Url Batch": {
		...
		"body": [
			...
			"",
			"-- ※Ki?m tra batch c? ?ang ch?y hay kh?ng",
			"ps -ef | grep batch",
			"",
			"-- shutdown",
			"sh batchsv.sh -s",
	...
	"Call sp in tool have use PXS_BATCH.p_WriteBatchLog": {
		...
		"body": [
			"",
			"-- PXS_BATCH.P_WRITEBATCHLOG L? H?M D?NG ?? GHI LOG KHI CH?Y BATCH",
			"",
			"PXS_BATCH.P_WRITEBATCHLOG('TRACE', TO_CHAR(CLOCK_TIMESTAMP(), 'YYYY/MM/DD HH24:MI:SS') || '[' || LS_PROC_CD || '] ' || LN_DATACOUNT || '件処理。');",
			"",
			"-- D? TR?N TOOL ( V? D? NH? SQL EXPLORER ) M?NH ?? G?I H?M PXS_USER.P_SETUSERINFO ?? KH?I T?O SESSION USER INFO TH? V?N B? L?I, V? TRONG H?M N?Y C? D?NG BATCH INFO",
			"-- N?N M?NH PH?I G?I TH?M H?M PXS_BATCH.P_SETBATCHINFO ?? KH?I T?O TH?NG TIN CHO BATCH",
			"",
	...
	"Convert DateTime To Character": {
		"prefix": "convertdatetimetocharacter",
		"body": [
	...
	"Check Error Import Pattern": {
		"prefix": "checkerrorimportpattern",
		"body": [
			"\n",
			"\t\t-- 発注実績物流センター不整合チェック",
	...
	"Alter table enable triggers column": {
		...
		"body": [
			"\n",
			"ALTER TABLE [t?n b?ng] ENABLE TRIGGER [t?n trigger]",
			"\n"
		],
	...
	"Alter table disable triggers column": {
		...
		"body": [
			"\n",
			"ALTER TABLE [t?n b?ng] DISABLE TRIGGER [t?n trigger]",
			"\n"
		],
	...
	"Query table store validation": {
		...
		"body": [
			"\n",
			"SELECT * FROM UMF_SCREENVALIDDEF WHERE VALIDNAME = ’UMFFileCreateDefValidation’;",
			"SELECT * FROM UMF_PROCDEF WHERE PROC_CD = ’UMFFileCreateDef’ AND FUNC_CD = ’STANDARD_VALIDATION’;",
			"\n"
		],
	...
	"Query table store lookup": {
		...
		"body": [
			"\n",
			"SELECT * FROM UMF_SCREENDEF WHERE FUNC_CD = ’STANDARD_LOOKUP’ AND SCREEN_CD = ’UMFFileCreateDef’;",
			"\n"
		],
	...
	"Accept CSV Update Error Data": {
		"prefix": "acceptcsvupdateerrordate",
		"body": [
			"\n",
			"\t\t\t\t\tPERFORM PXS_FILEUPLOAD.p_AcceptCSVUpdateErrorData (",
			"\t\t\t\t\t\tNULL,",
	...
	"Get Attribute Class By Item Store": {
		"prefix": "getattributeclassbyitemstore",
		"body": [
			"\n",
	...
	"Select Query Format": {
		"prefix": "select_querystandardformat",
		"body": [
			"\n",
	...
	"Coalesce format": {
		...
		"description": "Coalesce format: n?u tham s? th? nh?t l? NULL th? s? s? d?ng tham s? th? 2"
	},
	...
	"NULLIF format": {
		...
		"description": "NULLIF format: n?u tham s? th? nh?t b?ng tham s? th? 2 th? tr? v? NULL"
	},
	...
	"GREATEST format": {
		...
		"description": "GREATEST format: tr? v? gi? tr? l?n nh?t trong c?c gi? tr? truy?n v?o"
	},
	...
	"LEAST format": {
		...
		"description": "LEAST format: tr? v? gi? tr? nh? nh?t trong c?c gi? tr? truy?n v?o"
	},
	...
	"distinct on format": {
		...
		"description": "distinct on format: distinct theo c?t"
	},
	...
	"RANK format": {
		...
		"description": "RANK format : RANK s?p x?p th? h?ng, ??ng h?ng th? h?ng k? ti?p s? b? m?t, c? th? d?ng v?i OVER()"
	},
	...
	"DENSE_RANK format": {
		...
		"description": "DENSE_RANK format : x?p h?ng, ??ng h?ng th? h?ng k? ti?p v?n ???c hi?n th?, c? th? d?ng v?i OVER()."
	},
	...
	"GENERATE_SERIES format": {
		...
		"description": "GENERATE_SERIES format: tr? v? to - from + 1 d?ng c? gi? tr? t? from ??n to v? b??c nh?y l? step (t?o s? li?n ti?p t? -> ??n.), c? th? d?ng v?i OVER()"
	},
	...
	"repeat format": {
		...
		"description": "repeat format: l?p l?i, c? th? d?ng v?i OVER()"
	},
	...
	"trim format": {
		...
		"description": "trim format: x?a all space ??u v? cu?i string {TRIM(LEADING FROM string)}"
	},
	...
	"ltrim format": {
		...
		"description": "ltrim format: x?a k? t? b?n tr?i {LTRIM(string, [character]);}"
	},
	...
	"rtrim format": {
		...
		"description": "rtrim forma: x?a k? t? b?n ph?i {RTRIM(string, [character]);}"
	},
	...
	"btrim format": {
		...
		"description": "btrim format: x?a k? t? b?n tr?i v? ph?i {RTRIM(string, [character]);}"
	},
	...
	"concat format": {
		...
		"description": "concat format: n?i chu?i n?u chu?i b?ng r?ng th? v?n hi?n nh?ng char kh?c r?ng {CONCAT(str_1, str_2, ...)}"
	},
	...
	"concentratate format": {
		...
		"description": "concentratate format: n?i chu?i n?u chu?i b?ng r?ng th? chu?i tr? v? l? r?ng(NULL) {CONCAT(str_1, str_2, ...)}"
	},
	...
	"ROUND format": {
		...
		"description": "ROUND format: l?m tr?n s? bao nhi?u s? sau 0 { ROUND (source [ , n ] ) }"
	},
	...
	"Average format": {
		...
		"description": "Average format: t?nh trung b?nh theo c?t c? th? d?ng v?i OVER()"
	},
	...
	"Sum format": {
		...
		"description": "Sum format: t?nh t?ng theo c?t c? th? d?ng v?i OVER()"
	},
	...
	"ROW_NUMBER format": {
		...
		"description": "ROW_NUMBER format: ??nh s? th? t?, c? th? d?ng v?i OVER()"
	},
	...
	"count format": {
		...
		"description": "count format: ??m ph?n t? trong table count(1) th? null v?n t?nh, c?n count(column) th? null kh?ng t?nh, c? th? d?ng v?i OVER()"
	},
	...
	"min format": {
		...
		"description": "min format: l?y gi? tr? nh? nh?t theo c?t, c? th? d?ng v?i OVER()"
	},
	...
	"max format": {
		...
		"description": "max format: l?y gi? tr? l?n nh?t theo c?t, c? th? d?ng v?i OVER()"
	},
	...
	"FIRST_VALUE format": {
		...
		"description": "FIRST_VALUE format: l?y gi? tr? ??u ti?n theo group partition by theo c?t, c? th? d?ng v?i OVER()"
	},
	...
	"ARRAY_AGG format": {
		...
		"description": "ARRAY_AGG format: n?i c?c ph?n t? l?i th?nh 1 m?ng{ ARRAY_AGG(expression [ORDER BY [sort_expression {ASC | DESC}], [...]) }"
	},
	...
	"STRING_AGG format": {
		...
		"description": "STRING_AGG format: n?i column th?nh 1 chu?i c?ch nhau theo k? hi?u ph?a sau.{ STRING_AGG ( expression, separator [order_by_clause] ) }"
	},
	...
	"sum from previous row to current row format": {
		...
		"description": "sum from previous row to current row format : t?nh t?ng t? d?ng tr??c ?? ??n d?ng hi?n t?i"
	},
	...
	"Over format": {
		...
		"description": "Over format : OVER([PARTITION BY t?n_c?t] [ORDER BY t?n_c?t] [ROWS/RANGE BETWEEN from AND to) \n from/to c? th? l? UNBOUNDED PRECEDING ho?c N PRECEDING ho?c CURRENT ROW ho?c UNBOUNDED FOLLOWING ho?c N FOLLOWING"
	},
